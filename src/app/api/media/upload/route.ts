  ...
  try {
    ...
    if (file.type.startsWith('image/')) {
      ...
      try {
        ...
        const fullExifData = parser.parse();
        console.log('EXIF data:', {
          orientation: fullExifData.tags?.Orientation,
          dateTime: fullExifData.tags?.DateTime,
          dateTimeOriginal: fullExifData.tags?.DateTimeOriginal,
          createDate: fullExifData.tags?.CreateDate
        });

        // 撮影日時を取得（優先度: DateTimeOriginal > CreateDate > DateTime）
        const dateTimeOriginal = fullExifData.tags?.DateTimeOriginal;
        const createDate = fullExifData.tags?.CreateDate;
        const dateTime = fullExifData.tags?.DateTime;
        ...
        // 必要最小限のEXIFデータのみを保存
        exifData = {
          make: fullExifData.tags?.Make,
          model: fullExifData.tags?.Model,
          dateTimeOriginal: fullExifData.tags?.DateTimeOriginal,
          gps: fullExifData.tags?.GPSLatitude && fullExifData.tags?.GPSLongitude ? {
            latitude: fullExifData.tags.GPSLatitude,
            longitude: fullExifData.tags.GPSLongitude,
            altitude: fullExifData.tags?.GPSAltitude
          } : undefined
        };
        
        console.log('Captured at:', capturedAt);
        console.log('Filtered EXIF data:', exifData);
      } catch (error) {
